    ...
    return (
        <>
            <div className={`watched ${arrow ? 'watchedOn' : null}`}>
                <div className="outin" onClick={() => { setArrow(!arrow) }}>
                    <span className="material-symbols-outlined">
                        {arrow ? 'keyboard_double_arrow_right' : 'keyboard_double_arrow_left'}
                    </span>
                </div>
                <div className="watchedlist">
                            ...
                            recendly.map((a, i) => {
                                return (
                                    <div className="watch-1" key={i}>
                                        <img src={process.env.PUBLIC_URL + `/main${a + 1}.jpeg`} alt="최근본상품1" onClick={()=>{
                                            navigate(`/detail/${a}`)
                                        }}/>
                                    </div>
                                )
                    ...
                    <div className="controlbtn">
                        <Navbar.Brand href="#" className='toTop'>Top▲</Navbar.Brand>
                        <button onClick={() => {
                            navigate(`/recently`)
                        }}>더보기</button>
                    </div>
